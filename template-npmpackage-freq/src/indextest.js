...
class freqChart {
   ...
   * @returns 
   */
  initCulateItemData1(datas, typestr) {
    let chartWidths = this.chartWidth;
    let freqdiff = this.options.x_axis.end_freq - this.options.x_axis.start_freq;
    let freqStep = chartWidths / freqdiff;
    this.freqPxStep = freqdiff / chartWidths;
    ...
    let gridData = [];
    let trackGap = this.options.grid.track_gap || 0;
    let trackTitleHeight = typestr === "childwifi" ? (this.options.grid.track_title_height || 30) : 0;

    let trackHeight = (this.chartHeight - trackTitleHeight - (datas.length - 1) * trackGap) / datas.length;
    let rowHeight = trackHeight * this.scaleY;
    let rowTitleHeight = trackTitleHeight * this.scaleY;

    // 提取公共逻辑到函数
    const getStartEndX = (start_freq, end_freq) => {
        let start_x = (start_freq - this.options.x_axis.start_freq) * freqStep;
        let end_x = (end_freq - this.options.x_axis.start_freq) * freqStep;
        let width = end_x - start_x;
        return { start_x, end_x, width };
    };

    const setDataAttributes = (data, start_x, end_x, width, rowHeight, rowTitleHeight) => {
        data.start_x = start_x;
        data.end_x = end_x;
        data.width = width;
        data.height = rowHeight;
        data.x = start_x + this.offsetX + this.options.grid.left;
        data.y = this.options.grid.top + (rowHeight + trackGap) * data.item_id + this.offsetY + rowTitleHeight;
    };

    // 遍历数据
    for (let i = 0; i < datas.length; i++) {
        let rowarr = typestr === "childwifi" ? [datas[i].child_band_ranges] : datas[i];
        ...
        for (let d = 0; d < rowarr.length; d++) {
            ...
            for (let j = 0; j < rows.length; j++) {
                ...
                // 计算频率范围对应的X坐标
                let { start_x, end_x, width } = getStartEndX(data.start_freq, data.end_freq);
                let label_start_x = start_x;
                let label_end_x = end_x;
                let label_start_freq = data.start_freq;
                let label_end_freq = data.end_freq;

                // 处理频率范围边界情况
                if (start_x < 0 && end_x >= 0 && end_x <= this.options.x_axis.end_freq) {
                    start_x = 0;
                    width = end_x - start_x;
                    label_start_freq = this.options.x_axis.start_freq;
                } else if (start_x >= 0 && start_x <= this.options.x_axis.end_freq && end_x > this.options.x_axis.end_freq) {
                    end_x = (this.options.x_axis.end_freq - this.options.x_axis.start_freq) * freqStep;
                    width = end_x - start_x;
                    label_end_x = end_x;
                    label_end_freq = this.options.x_axis.end_freq;
                } else if (start_x < 0 && end_x > this.options.x_axis.end_freq) {
                    start_x = 0;
                    end_x = (this.options.x_axis.end_freq - this.options.x_axis.start_freq) * freqStep;
                    width = end_x - start_x;
                    label_start_freq = this.options.x_axis.start_freq;
                ...
                // 更新数据属性
                data.label_start_x = label_start_x;
                data.label_start_freq = label_start_freq;
                data.label_end_x = label_end_x;
                data.label_end_freq = label_end_freq;
                setDataAttributes(data, start_x, end_x, width, rowHeight, rowTitleHeight);

                // 根据类型更新颜色和文本
                if (this.options.type === 3) {
                    data.color = data.color;
                    let texts = data.language.find(item => item.code === this.options.language_code)?.name || "";
                    data.text = texts;
                } else if (this.options.type === 4) {
                    data.color = this.getItemColor(data.business_id);
                    let texts = this.getItemText(data.business_id);
                    data.text = texts + data.sub_title;
                ...
                if (this.options.type === 2 && data.child_band) {
                    ...
                    childTreeData.forEach((item) => {
                        Object.assign(item, {
                            ...
                            end_x: data.end_x,
                            parent_orderid: data.orderid,
                            parent_foot_notes: data.foot_notes,
                            child_band_ranges: this.getRegionData(item.child_band_range, this.options.x_axis.start_freq, this.options.x_axis.end_freq)
                        });
                    });
  ...
  initCulateItemData(datas, typestr) {
    ...
    if (typestr == "childwifi") {
      trackGap = 0;
      trackHeight = (this.chartHeight - (datas.length - 1) * trackGap) / (datas.length+1); //行高
    }
   ...
   */
  initBackground() {
    let fillcolor = colorTostring(this.options.background)
    ...
    for (let i = 0; i < this.treeData.length; i++) {
      ...
      for (let j = 0; j < itemArray.length; j++) {
        ...
        if(this.options.type == 2&&this.options.channel_show=="show"){
          ...
          this.drawText(row.sub_title, x, y, width, this.options.grid.track_title_height);
  ...
  setSwitch(event) {
    event.stopPropagation(); // 阻止事件冒泡
    this.focusInfo.focus = this.focusInfo.list[0][1]
    this.focusInfo.prefocus = this.focusInfo.list[0][1];
    //this.focusInfo.focus = null;
    this.drawChart(); // 重新绘制图表
  ...
  setKey(datas) {
    ...
    if (this.focusInfo.focus == null) {
      ...
      if (this.focusInfo.focus.type == "chart") {
        ...
        // else if(this.focusInfo.focus.key=="zoomout"){
