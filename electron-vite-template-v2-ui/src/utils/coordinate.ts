 ...
 * @returns {boolean}
 */
export function out_of_china(lng: any, lat: any) {
    var lat: any = +lat;
    var lng: any = +lng;
...
export function transformlat(lng: any, lat: any) {
    var lat: any = +lat;
    var lng: any = +lng;
...
export function transformlng(lng: any, lat: any) {
    var lat: any = +lat;
    var lng: any = +lng;
...
export function gcj02tobd09(lng:any, lat:any) {
...
export function wgs84tobd09(lng:any, lat:any){
    let tempGeom = wgs84togcj02(lng, lat);

    return gcj02tobd09(tempGeom[0], tempGeom[1])
}
export function getTime(type: string) {
    const currentTime = new Date();
    const weekDay = '周' + '天一二三四五六'.charAt(currentTime.getDay()); // 返回的是0-6，0表示星期日，1表示星期一，以此类推
    const year = currentTime.getFullYear();
    const month = String(currentTime.getMonth() + 1).padStart(2, '0'); // 月份从0开始，因此需要加1
...
export function cloneObjectFn(obj: any) {
    return JSON.parse(JSON.stringify(obj))
}

// 处理数据为", "格式
export function formattedData(msd: any) {
    const newDate = msd.split("=").reduce((obj: any, pair: any) => {
        let parts = "";
        const newPair = pair.split(",");
        if (newPair.length === 1) {
            parts = pair.trim();
        } else if (newPair.length === 2) {
            parts = newPair[0].trim() + ", " + newPair[1].trim();
        } else {
            const firstPart = newPair[0].trim() + "," + newPair[1].trim();
            const secondPart = newPair.slice(2).join(",");
            parts = firstPart + ", " + secondPart.trim();
        }
        obj = [obj, parts].join("=");
        return obj;
    }, "");
...
// 百度地图坐标偏移计算
var forEachPoint = function (func: any) {
    return function (input: any, opt_output: any, opt_dimension: any) {
...
sphericalMercator.forward = forEachPoint(function (input: any, output: any, offset: any) {
...
sphericalMercator.inverse = forEachPoint(function (input: any, output: any, offset: any) {
...
function getRange(v: any, min: any, max: any) {
    v = Math.max(v, min);
    v = Math.min(v, max);
    return v;
}

function getLoop(v: any, min: any, max: any) {
    var d = max - min;
    while (v > max) {
        v -= d;
    }
...
function convertor(input: any, output: any, offset: any, table: any) {
    var px = input[offset];
...
baiduMercator.forward = forEachPoint(function (input: any, output: any, offset: any) {
...
baiduMercator.inverse = forEachPoint(function (input: any, output: any, offset: any) {
...
function delta(wgLon: any, wgLat: any) {
...
gcj02.toWGS84 = forEachPoint(function (input: any, output: any, offset: any) {
...
gcj02.fromWGS84 = forEachPoint(function (input: any, output: any, offset: any) {
    var lng = input[offset];
    var lat = input[offset + 1];
    if (!outOfChina(lng, lat)) {
        var deltaD = delta(lng, lat);
...
function toGCJ02(input: any, output: any, offset: any) {
...
function fromGCJ02(input: any, output: any, offset: any) {
    var x = input[offset];
...
bd09.toWGS84 = function (input: any, opt_output: any, opt_dimension: any) {
    var output = forEachPoint(toGCJ02)(input, opt_output, opt_dimension);
    return gcj02.toWGS84(output, output, opt_dimension);
};

bd09.fromWGS84 = function (input: any, opt_output: any, opt_dimension: any) {
